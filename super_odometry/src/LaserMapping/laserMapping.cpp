...
namespace super_odometry
{
    ...
    void laserMapping::initInterface()
    {
        ...
        pubOdomBaseLink = this->create_publisher<nav_msgs::msg::Odometry>(ProjectName + "/laser_odometry_base_link", 1);
    ...
    bool laserMapping::readParameters()
    {
        ...
        this->declare_parameter("laser_mapping_node.publish_base_link", true);
        this->declare_parameter("laser_mapping_node.base_frame", "base_link");
        ...
        config_.publish_base_link = this->get_parameter("laser_mapping_node.publish_base_link").as_bool();
        config_.base_frame = this->get_parameter("laser_mapping_node.base_frame").as_string();
    ...
    void laserMapping::initializeFirstFrame()
    {
        ...
        if (sensorMeas.imuPrediction.w() != 0)
        { // Have IMU data
    ...
    void laserMapping::publishTopic()
    {
        ...
        if (config_.publish_base_link)
        {
            publishBaseLinkOdometry(odomAftMapped);
        }
    ...
    {
        return !cornerLastBuf.empty() && !surfLastBuf.empty() && !fullResBuf.empty() && !IMUPredictionBuf.empty();
    ...
    void laserMapping::performSLAMOptimization()
    {
        ...
        { // TODO: Livox mid360 not use roll pitch angle
    ...
    void laserMapping::publishBaseLinkOdometry(const nav_msgs::msg::Odometry &sensor_odom)
    {
        // 1. Copy the original sensor odometry (keeps header, stamp, covariance, etc.)
        nav_msgs::msg::Odometry odom_base_link = sensor_odom;

        geometry_msgs::msg::TransformStamped tf_sensor_to_base;
        bool use_base_link_tf = false;

        // 2. Look up the TF for the Turret/Base
        try
        {
            tf_sensor_to_base = tf_buffer_->lookupTransform(
                SENSOR_FRAME, config_.base_frame,
                sensor_odom.header.stamp, rclcpp::Duration::from_seconds(0.05));
            use_base_link_tf = true;
        }
        catch (const tf2::ExtrapolationException &ex)
        {
            RCLCPP_INFO(this->get_logger(), "TF Extrapolation. Falling back to latest available transform.");
            try
            {
                tf_sensor_to_base = tf_buffer_->lookupTransform(
                    SENSOR_FRAME, config_.base_frame, tf2::TimePointZero);
                use_base_link_tf = true;
            }
            catch (const tf2::TransformException &ex2)
            {
                RCLCPP_INFO(this->get_logger(), "Could not transform SENSOR to BASE_LINK (latest): %s", ex2.what());
            }
        }
        catch (const tf2::TransformException &ex)
        {
            RCLCPP_INFO(this->get_logger(), "Could not transform SENSOR to BASE_LINK (exact): %s", ex.what());
        }

        // 3. Apply math and publish ONLY if TF was found
        if (use_base_link_tf)
        {
            odom_base_link.child_frame_id = config_.base_frame;

            // Convert TF message to Eigen
            Eigen::Quaterniond q_sensor_base(
                tf_sensor_to_base.transform.rotation.w, tf_sensor_to_base.transform.rotation.x,
                tf_sensor_to_base.transform.rotation.y, tf_sensor_to_base.transform.rotation.z);
            Eigen::Vector3d t_sensor_base(
                tf_sensor_to_base.transform.translation.x, tf_sensor_to_base.transform.translation.y,
                tf_sensor_to_base.transform.translation.z);

            // Calculate Base Link Pose
            Transformd T_map_base = Transformd(q_w_curr, t_w_curr) * Transformd(q_sensor_base, t_sensor_base);

            // Rotate Velocities to Base Link Frame
            Eigen::Vector3d vel_base = q_sensor_base.inverse() * vel_b;
            Eigen::Vector3d ang_vel_base = q_sensor_base.inverse() * ang_vel_b;

            // Overwrite the copied message with the shifted coordinates
            odom_base_link.pose.pose.orientation.x = T_map_base.rot.x();
            odom_base_link.pose.pose.orientation.y = T_map_base.rot.y();
            odom_base_link.pose.pose.orientation.z = T_map_base.rot.z();
            odom_base_link.pose.pose.orientation.w = T_map_base.rot.w();

            odom_base_link.pose.pose.position.x = T_map_base.pos.x();
            odom_base_link.pose.pose.position.y = T_map_base.pos.y();
            odom_base_link.pose.pose.position.z = T_map_base.pos.z();

            odom_base_link.twist.twist.linear.x = vel_base.x();
            odom_base_link.twist.twist.linear.y = vel_base.y();
            odom_base_link.twist.twist.linear.z = vel_base.z();

            odom_base_link.twist.twist.angular.x = ang_vel_base.x();
            odom_base_link.twist.twist.angular.y = ang_vel_base.y();
            odom_base_link.twist.twist.angular.z = ang_vel_base.z();

            // PUBLISH TO YOUR NEW DEDICATED TOPIC
            pubOdomBaseLink->publish(odom_base_link);
        }
        else
        {
            // If the TF drops out, it's better to not publish the base_link odometry
            // than to publish wrong data to the UKF.
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                                 "Skipping base_link odometry publish due to missing TF.");
        }
    }
